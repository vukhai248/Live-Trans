...
| **ISSUE-014** | CI / Linting (GitHub Actions) | GitHub Actions CI liên tục báo đỏ (Fail) sau mỗi lần push dù code chạy bình thường; job bị ngắt ở bước `npm run lint` (`eslint .`). | ESLint v9 có luật ngặt nghèo (`no-empty` cấm catch rỗng, `no-control-regex` cấm regex ký tự điều khiển cần thiết cho bóc tách PDF font rác); tồn đọng 28 cảnh báo biến thừa (`no-unused-vars`), gán thừa (`no-useless-assignment`) và escape regex thừa (`no-useless-escape`). | (1) Bật `allowEmptyCatch: true` và tắt `no-control-regex` trong `eslint.config.mjs`; (2) Dọn sạch các biến thừa, gán đè và ký tự escape dư thừa trên toàn bộ codebase; (3) Xác thực `npm run check` xanh 100% (0 errors, 0 warnings). | ✅ Đã khắc phục & CI Xanh |
